...
#if defined (WIN32) || defined (__APPLE__) || defined(__NetBSD__) // MINGW/MSYS, NetBSD, and MacOSX/Iphone need it, at least for now
...
#ifdef __IPHONEOS__
static Uint32 last_opt_motion_tick;
static Sint32 last_ropt_x, last_ropt_y;
static SDL_Event IOSTouchScrollEvent;
...
static void setup_normal_screen_layout(void)
{
  ...
  r_tcolors.x = x_offset;
  r_tcolors.w = gd_tools.cols * button_w;
  r_colors.x = r_tcolors.w + x_offset;
  r_colors.w = WINDOW_WIDTH - r_tcolors.w - x_offset;
  ...
  r_canvas.x = gd_tools.cols * button_w + x_offset;
  r_canvas.y = 0;
  r_canvas.w = WINDOW_WIDTH - (gd_tools.cols + gd_toolopt.cols) * button_w - x_offset;

  r_tuxarea.x = x_offset;
  r_tuxarea.w = WINDOW_WIDTH - x_offset;
...
static float retina_screen_scale = 1;
...
void SDL_UpdateRect(SDL_Surface *inscreen, Sint32 x, Sint32 y, Sint32 w, Sint32 h)
{
    ...
    if (inscreen == screen) {
        ...
        r.w = w;
        r.h = h;
        dr.x = x * screen_scale * retina_screen_scale;
        dr.y = y * screen_scale * retina_screen_scale;
        dr.w = w * screen_scale * retina_screen_scale;
        dr.h = h * screen_scale * retina_screen_scale;
        if (SDL_MUSTLOCK(screen)) {
            SDL_LockSurface(screen);
            SDL_UpdateTexture(sdltexture, &r, screen->pixels + y * screen->pitch + x * screen->format->BytesPerPixel, screen->pitch);
            SDL_UnlockSurface(screen);
        } else {
            SDL_UpdateTexture(sdltexture, &r, screen->pixels + y * screen->pitch + x * screen->format->BytesPerPixel, screen->pitch);
        }
        SDL_RenderCopy(sdlrenderer, sdltexture, &r, &dr);
        SDL_RenderPresent(sdlrenderer);
    }
}
SDL_Surface *SDL_DisplayFormatAlpha(SDL_Surface *surface)
{
    ...
    SDL_GetWindowDisplayMode(sdlWindow, &displaymode);
    return SDL_ConvertSurfaceFormat(surface, displaymode.format, 0);
}
...
#if  defined(NOKIA_770) || defined(__IPHONEOS__)
static int fullscreen = 1;
#else
static int fullscreen;
#endif
static int native_screensize = 1;
...
static int disable_quit = 1; //For IphoneOS MArk Tu

static int noshortcuts;
static int disable_save;
static int ok_to_use_lockfile = 0; //Mark Tu
...
static void handle_keymouse(SDL_Keycode key, int updown, int steps, SDL_Rect *area1, SDL_Rect *area2);
...
//            printf("event:0x%x\n", event.type);
//        }
        if ((screen_scale != 1) && ((event.type == SDL_MOUSEBUTTONDOWN) || (event.type == SDL_MOUSEBUTTONUP) || (event.type == SDL_MOUSEMOTION))) {
            event.button.x =  (float)event.button.x / screen_scale;
            event.button.y =  (float)event.button.y / screen_scale;
        }
        //printf("et:%x\n", event.type);
...
//                        playsound(screen, 0, SND_DELETE_LABEL, 0, SNDPOS_CENTER); // FIXME lack of specific sound
        ...
        {
//            static int redraw = 0;
            char* im_cp = (char *)&event.key.keysym.sym;
...
//            /* Read IM, remember how many to redraw next iteration */
//            redraw = im_read(&im_data, event.text.text);
//            printf("texinput %04x----%04x, %lu\n", *(wchar_t *)event.text.text, *im_data.s, sizeof(wchar_t));
//           int trailing_bytes = 0;
      ...
      else if (event.type == SDL_TEXTINPUT)
      {
              ...
              (cur_tool == TOOL_LABEL && cur_label == LABEL_SELECT))
          {
              ...
              /* Discard previous # of redraw characters */
              /*if((int)texttool_len <= redraw) texttool_len = 0;
              else texttool_len -= redraw;*/ //MarkTu
              texttool_len = 0; //MarkTu
              texttool_str[texttool_len] = '\0';
              ...
              /* Queue each character to be displayed */
              while(*im_cp) {
                  unsigned char c = *((unsigned char *)im_cp);
                  if (UTF8_IsLeadByte(c)) {
                      trailing_bytes = UTF8_TrailingBytes(c);
                  } else if (c <= 0x7f) {
                      trailing_bytes = 0;
                  } else if(!trailing_bytes) {
                      goto out; //garbage
                  }
                  ...
                  else if ((isprint(*im_cp) || trailing_bytes) &&
                           (cur_tool == TOOL_TEXT || cur_label == LABEL_LABEL))
                  {
                      if (texttool_len < (sizeof(texttool_str) / sizeof(wchar_t)) - 1)
                      {
	  ...
	  if (HIT(real_r_tools))
	{
	  ...
	  whicht = tool_scroll + GRIDHIT_GD(real_r_tools, gd_tools);
        ...
        if (whicht < NUM_TOOLS && tool_avail[whicht] &&
            valid_click(event.button.button) && cur_tool != whicht)
        {
            IOSFlurryCustomevent(tool_names[whicht], tool_names[cur_tool]);
        }
#endif
	  if (whicht < NUM_TOOLS && tool_avail[whicht] &&
	      (valid_click(event.button.button) || whicht == TOOL_PRINT))
	  {
	    ...
	    if ((cur_tool == TOOL_TEXT && whicht != TOOL_TEXT &&
	       ...
	       (cur_tool == TOOL_LABEL && whicht != TOOL_LABEL &&
                ...
                whicht != TOOL_QUIT))
	    {
            IOSStopTextInput(sdlWindow, 0, 0);
	      if (cursor_x != -1 && cursor_y != -1)
	      {
		hide_blinking_cursor();
		if (texttool_len > 0)
		...
		if (onscreen_keyboard && kbd)
		{
	      ...
	      cur_tool = old_tool;
	      draw_toolbar();
	      draw_tux_text(TUX_BORED, gettext_noop("image is saved to your photo sucessfully"), 0);
	      update_screen_rect(&r_tools);
	    }
        else if (cur_tool == TOOL_SHARE)
        {
            /* Assemble drawing plus any labels: */
            /*SDL_Color black = { 0, 0, 0, 0 };
            SDL_Surface *pwr_text = render_text(locale_font, gettext("Powered by TuxPaint"), black);
            SDL_BlitSurface(canvas, NULL, save_canvas, NULL);
            SDL_BlitSurface(label, NULL, save_canvas, NULL);
            SDL_Rect sr = {0, 0, pwr_text->w, pwr_text->h}, dr = {save_canvas->w-pwr_text->w-20, save_canvas->h-pwr_text->h-20, pwr_text->w, pwr_text->h};
            SDL_BlitSurface(pwr_text, &sr, save_canvas, &dr);*/
            IOSSharing(save_canvas, 0, screen->w, screen->h);
        }
	...
	else if (HIT(r_toolopt) && valid_click(event.button.button))
	{
	  ...
	  //          magic_switchout(canvas);
#ifdef __IPHONEOS__
        if(!last_opt_motion_tick) {
            last_opt_motion_tick = SDL_GetTicks();
            last_ropt_x = event.button.x;
            last_ropt_y = event.button.y;
        }
#endif
...
*/
	...
	else if (HIT(r_canvas) && valid_click(event.button.button) && keyglobal == 0)
	{
	  ...
	  else if (cur_tool == TOOL_TEXT || cur_tool == TOOL_LABEL)
	  {
        ...
        IOSSTartTextInput(sdlWindow, (float)event.button.x*screen_scale, (float)event.button.y*screen_scale, texttool_str);
	...
	else if (event.user.code == USEREVENT_PLAYDESCSOUND)
	{
      ...
      {
#ifdef __IPHONEOS__
          last_opt_motion_tick = 0;
          last_ropt_x = last_ropt_y = 0;
#endif
	if (scrolling)
	{
	  if (scrolltimer != 0)
	...
	oldpos_y = event.motion.y;
          ...
          if (last_opt_motion_tick) {
              //Uint32 tick_delta = ticks -(ticks < last_opt_motion_tick ? 0 : last_opt_motion_tick);
              Sint32 DeltaY = event.motion.y - last_ropt_y;
              Sint32 DeltaX = event.motion.x - last_ropt_x;
              //printf("dx:%d, dy:%d\n", DeltaX, DeltaY);
//              float speedy = (float)(event.motion.y - last_ropt_y) / (float)tick_delta;
//              float speedx = (float)(event.motion.x - last_ropt_x) / (float)tick_delta;
//              float sspd = ((float)WINDOW_HEIGHT/(float)3000);
              //        if (stepsy > 3) {
              //            stepsy = 3;
              //        } else if (stepsy < -3) stepsy = -3;
              if(DeltaY > button_h)
              {
                  IOSTouchScrollEvent.button.type = SDL_MOUSEBUTTONDOWN;
                  IOSTouchScrollEvent.button.windowID = event.motion.windowID;
                  IOSTouchScrollEvent.button.which = event.motion.which;
                  IOSTouchScrollEvent.button.state = event.motion.state;
                  IOSTouchScrollEvent.button.button = 4;
                  IOSTouchScrollEvent.button.clicks = 1;
                  IOSTouchScrollEvent.button.x = event.motion.x * screen_scale;
                  IOSTouchScrollEvent.button.y = event.motion.y * screen_scale;
                  SDL_PushEvent(&IOSTouchScrollEvent);
                  last_opt_motion_tick = ticks;
                  
                  last_ropt_y = event.motion.y;
              } else if (DeltaY < -button_h)
              {
                  IOSTouchScrollEvent.button.type = SDL_MOUSEBUTTONDOWN;
                  IOSTouchScrollEvent.button.windowID = event.motion.windowID;
                  IOSTouchScrollEvent.button.which = event.motion.which;
                  IOSTouchScrollEvent.button.state = event.motion.state;
                  IOSTouchScrollEvent.button.button = 5;
                  IOSTouchScrollEvent.button.clicks = 1;
                  IOSTouchScrollEvent.button.x = event.motion.x * screen_scale;
                  IOSTouchScrollEvent.button.y = event.motion.y * screen_scale;
                  SDL_PushEvent(&IOSTouchScrollEvent);
              ...
              if (!disable_stamp_controls && (cur_tool == TOOL_STAMP)) {
                  int control_sound = -1;
                  if (DeltaX > r_toolopt.w) {
                      // right button for stamps
                      
                      /* Prev/Next Controls: */
                      int step = 0;
                      for (step = DeltaX; step > r_toolopt.w; step -= r_toolopt.w) {
                          old_stamp_group = stamp_group;
                      ...
                      /*IOSTouchScrollEvent.button.type = SDL_MOUSEBUTTONDOWN;
                       ...
                       IOSTouchScrollEvent.button.which = event.motion.which;
                       IOSTouchScrollEvent.button.state = event.motion.state;
                       IOSTouchScrollEvent.button.button = 1;
                       IOSTouchScrollEvent.button.clicks = 1;
                       IOSTouchScrollEvent.button.x = (r_toolopt.x + r_toolopt.w*3/4) * screen_scale;
                       IOSTouchScrollEvent.button.y = (r_toolopt.y + r_toolopt.h - button_h*2.5) * screen_scale;
                       SDL_PushEvent(&IOSTouchScrollEvent);*/
                      last_opt_motion_tick = ticks;
                      last_ropt_x = event.motion.x - step;
                      
                  } else
                      if (DeltaX < -r_toolopt.w) {
                          //printf("----x:%d  lx:%d, dx:%d \n", event.motion.x, last_ropt_x, DeltaX);
                          int step = 0;
                          for (step = -DeltaX; step > r_toolopt.w; step -= r_toolopt.w) {
                              old_stamp_group = stamp_group;
                              /* Prev group */
                          ...
                          /*IOSTouchScrollEvent.button.type = SDL_MOUSEBUTTONDOWN;
                           ...
                           IOSTouchScrollEvent.button.button = 1;
                           IOSTouchScrollEvent.button.clicks = 1;
                           IOSTouchScrollEvent.button.x = (r_toolopt.x + r_toolopt.w/2) * screen_scale;
                           IOSTouchScrollEvent.button.y = (r_toolopt.y + r_toolopt.h - button_h*2.5) * screen_scale;
                           SDL_PushEvent(&IOSTouchScrollEvent);*/
                          last_opt_motion_tick = ticks;
                          last_ropt_x = event.motion.x + step;;
...
#elif __IPHONEOS__
  printf("  Built for IOS  (__IPHONEOS__)\n");
#else
...
#if !defined(WIN32) && !defined(__MACOSX__) && !defined(__BEOS__) && !defined(__HAIKU__)
...
static void draw_toolbar(void)
{
 ...
 gd_tools.rows = max / 2;
  ...
  {
    i = tool - tool_scroll;
    dest.x = ((i % 2) * 48) + x_offset;
    dest.y = ((i / 2) * 48) + 40 + off_y;

#ifdef __IPHONEOS__
    if (tool < NUM_TOOLS && tool != TOOL_QUIT)
#else
    if (tool < NUM_TOOLS)
#endif
    {
	...
	button_color = img_grey;
      ...
      dest.x = ((i % 2) * 48) + 4 + x_offset;
      dest.y = ((i / 2) * 48) + 40 + 2 + off_y;

      SDL_BlitSurface(img_tools[tool], NULL, screen, &dest);


      dest.x = ((i % 2) * 48) + 4 + (40 - img_tool_names[tool]->w) / 2 + x_offset;
			     ...
			     int want_right_to_left, Uint8 locale_text)
{
  ...
  control_drawtext_timer(0, empty, 0);
...
static SDL_Surface * load_starter_helper(char * path_and_basename, char * extension, SDL_Surface * (*load_func)(const char *)) {
  char * ext;
  char fname[256];
  SDL_Surface * surf;
  int i;
				     ...
				     int ox, int oy)
{
  ...
  playsound(screen, 1, snd, 1, SNDPOS_LEFT, SNDDIST_NEAR);
    ...
    int flags = 0;
#endif
  alpha_surf = SDL_CreateRGBSurface(flags,
				    (PROMPT_W - 96 * 2) + (w - 4) * 2,
				    (w - 4) * 2,
				    screen->format->BitsPerPixel,
				    screen->format->Rmask,
			   ...
			   size_t dataLen)
{
  ...
  sprintf(line, "%zu%s", uncompressed_size, "\n");
  strcat(headers, line);
  sprintf(line, "%zu%s", dataLen, "\n");
  strcat(headers, line);
...
#elif defined(__IPHONEOS__)
    SurfaceSaveToPHOTO(save_canvas, screen->w, screen->h);
    //do_prompt_snd(gettext(), PROMPT_PRINT_YES, "", SND_TUXOK, 0, 0);
#endif
...
void IOSMisc_StartAD(SDL_Window * sdlWindow, float x, float y, float w, float h);
void IOSInitMisc();
...
#define DECLARE_MAGIC_FUNC(PN)\
    int magic_command(PN,get_tool_count)(magic_api *);\
    char * magic_command(PN,get_name)(magic_api *, int);\
    SDL_Surface * magic_command(PN,get_icon)(magic_api *, int);\
    char * magic_command(PN,get_description)(magic_api *, int, int);\
    int magic_command(PN,requires_colors)(magic_api *, int);\
    int magic_command(PN,modes)(magic_api *, int);\
    void magic_command(PN,set_color)(magic_api *, Uint8, Uint8, Uint8);\
    int magic_command(PN,init)(magic_api *);\
    Uint32 magic_command(PN,api_version)(void);\
    void magic_command(PN,shutdown)(magic_api *);\
    void magic_command(PN,click)(magic_api *, int, int, SDL_Surface *, SDL_Surface *, int, int, SDL_Rect *);\
    void magic_command(PN,drag)(magic_api *, int, SDL_Surface *, SDL_Surface *, int, int, int, int, SDL_Rect *);\
    void magic_command(PN,release)(magic_api *, int, SDL_Surface *, SDL_Surface *, int, int, SDL_Rect *);\
    void magic_command(PN,switchin)(magic_api *, int, int, SDL_Surface *, SDL_Surface *);\
    void magic_command(PN,switchout)(magic_api *, int, int, SDL_Surface *, SDL_Surface *);

#define MAGIC_FUNC_POINTER(PN) \
    DECLARE_MAGIC_FUNC(PN) \
    magic_funcs[PN].get_tool_count = magic_command(PN, get_tool_count); \
    magic_funcs[PN].get_name = magic_command(PN, get_name);\
    magic_funcs[PN].get_icon = magic_command(PN, get_icon);\
    magic_funcs[PN].get_description = magic_command(PN, get_description);\
    magic_funcs[PN].requires_colors = magic_command(PN, requires_colors);\
    magic_funcs[PN].modes= magic_command(PN, modes);\
    magic_funcs[PN].set_color = magic_command(PN, set_color);\
    magic_funcs[PN].init = magic_command(PN, init);\
    magic_funcs[PN].api_version = magic_command(PN, api_version);\
    magic_funcs[PN].shutdown = magic_command(PN, shutdown);\
    magic_funcs[PN].click = magic_command(PN, click);\
    magic_funcs[PN].drag = magic_command(PN, drag);\
    magic_funcs[PN].release = magic_command(PN, release);\
    magic_funcs[PN].switchin = magic_command(PN, switchin);\
    magic_funcs[PN].switchout = magic_command(PN, switchout);
static void load_magic_plugins(void)
{
    
    int num_plugin_files = 0;
    int res, n, i, plc = MAGIC_PLACE_GLOBAL;
    ...
    MAGIC_FUNC_POINTER(alien)
    MAGIC_FUNC_POINTER(blind)
    MAGIC_FUNC_POINTER(blocks_chalk_drip)
    MAGIC_FUNC_POINTER(blur)
    MAGIC_FUNC_POINTER(bricks)
    MAGIC_FUNC_POINTER(calligraphy)
    MAGIC_FUNC_POINTER(cartoon)
    MAGIC_FUNC_POINTER(confetti)
    MAGIC_FUNC_POINTER(distortion)
    MAGIC_FUNC_POINTER(emboss)
    MAGIC_FUNC_POINTER(fade_darken)
    MAGIC_FUNC_POINTER(fill)
    MAGIC_FUNC_POINTER(fisheye)
    MAGIC_FUNC_POINTER(flower)
    MAGIC_FUNC_POINTER(foam)
    MAGIC_FUNC_POINTER(fold)
    MAGIC_FUNC_POINTER(fretwork)
    MAGIC_FUNC_POINTER(glasstile)
    MAGIC_FUNC_POINTER(grass)
    MAGIC_FUNC_POINTER(halftone)
    MAGIC_FUNC_POINTER(kalidescope)
    MAGIC_FUNC_POINTER(light)
    MAGIC_FUNC_POINTER(metalpaint)
    MAGIC_FUNC_POINTER(mirror_flip)
    MAGIC_FUNC_POINTER(mosaic)
    MAGIC_FUNC_POINTER(mosaic_shaped)
    MAGIC_FUNC_POINTER(negative)
    MAGIC_FUNC_POINTER(noise)
    MAGIC_FUNC_POINTER(perspective)
    MAGIC_FUNC_POINTER(puzzle)
    MAGIC_FUNC_POINTER(rails)
    MAGIC_FUNC_POINTER(rain)
    MAGIC_FUNC_POINTER(rainbow)
    MAGIC_FUNC_POINTER(realrainbow)
    MAGIC_FUNC_POINTER(ripples)
    MAGIC_FUNC_POINTER(rosette)
    MAGIC_FUNC_POINTER(sharpen)
    MAGIC_FUNC_POINTER(shift)
    MAGIC_FUNC_POINTER(smudge)
    MAGIC_FUNC_POINTER(snow)
    MAGIC_FUNC_POINTER(string)
    MAGIC_FUNC_POINTER(tint)
    MAGIC_FUNC_POINTER(toothpaste)
    MAGIC_FUNC_POINTER(tornado)
    MAGIC_FUNC_POINTER(tv)
    MAGIC_FUNC_POINTER(waves)
    MAGIC_FUNC_POINTER(xor)
    
    for (num_plugin_files = 0; num_plugin_files < plugin_num; num_plugin_files++) {
        ...
        if (n == 0)
        {
            fprintf(stderr, "Error: plugin %d failed to startup or reported 0 magic tools\n", num_plugin_files);
            fflush(stderr);
        }
        else
        {
...
#if defined(__MACOSX__)  //EP added this conditional section for Mac to allow for a config in the current directory, that supersedes sys and user configs
...
#elif __IPHONEOS__
      extern char *ios_doc_dir;
      IOSInitMisc();
      savedir = ios_doc_dir;//"../Documents/save";
      //savedir = "./Documents/";
#else
...
#if defined(__MACOSX__)  //EP added this conditional section for Mac
...
#elif defined(__MACOSX__)        //EP added this conditional section for Mac to fix folder&extension inconsistency with Tux Paint Config application)
  ...
  if(tmpcfg.joystick_buttons_ignore)
    {
	  ...
	  /* FIXME: Find better exit code */
	  printf("Joystick buttons must be between 0 and 254 :%s", tmpcfg.joystick_button_print);
	  exit(1);
        }
...
#if defined(__BEOS__) || defined(WIN32) || defined(__MACOSX__)   //EP added __MACOSX__
...
#if defined(__MACOSX__)  //EP added to fix 10.9 issue of current directory set by Finder to something else than folder where app bundle resides
...
#else
int TP_EventFilter (void *userdata, SDL_Event * event)
{
    if (event->type == SDL_QUIT ||
...
static void setup(void)
{
  ...
  if (fullscreen)
  {
    ...
    else
    {
      ...
      sdlWindow = SDL_CreateWindow(NULL, SDL_WINDOWPOS_UNDEFINED,
                                 SDL_WINDOWPOS_UNDEFINED,
                                 0, 0, SDL_WINDOW_FULLSCREEN_DESKTOP|SDL_WINDOW_ALLOW_HIGHDPI);
      SDL_GetWindowSize(sdlWindow, &WINDOW_WIDTH, &WINDOW_HEIGHT);
      
#ifdef __IPHONEOS__//for latest SDL2 version
      /*float scale =  IOSReturnSceenScale();
      WINDOW_WIDTH *= scale;
      WINDOW_HEIGHT *= scale;*/
      retina_screen_scale =IOSReturnSceenScale();
      Uint32 notch = 0;
      if (IOSHasNotch()){
          notch |= SDL_WINDOW_CUT_NOTCH;
      }
#endif
      if (notch) WINDOW_WIDTH -= 50;
      if (WINDOW_WIDTH > 1200) {
          screen_scale = (float) WINDOW_WIDTH / 1200.0;
          WINDOW_WIDTH = 1200;
          WINDOW_HEIGHT = (float)WINDOW_HEIGHT / screen_scale;
      } else if (WINDOW_HEIGHT < 600){
          screen_scale = (float) WINDOW_HEIGHT / 600.0;
          WINDOW_HEIGHT = 600;
          WINDOW_WIDTH = (float) WINDOW_WIDTH / screen_scale;
      }
      else screen_scale = 1;
      ...
      sdlWindow = SDL_CreateWindow(NULL, SDL_WINDOWPOS_UNDEFINED,
                                   SDL_WINDOWPOS_UNDEFINED,
                                   WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_BORDERLESS|SDL_WINDOW_ALLOW_HIGHDPI | notch);

      sdlrenderer = SDL_CreateRenderer(sdlWindow, -1, 0);
      //SDL_RenderSetScale(sdlrenderer, screen_scale, screen_scale);

      if ((sdlWindow == NULL) || (sdlrenderer == NULL)) {
          fprintf(stderr,
                  "\nWarning: couldn't create window and render.\n"
                  "The Simple DirectMedia Layer error that occurred was:\n"
                  "%s\n\n", SDL_GetError());
      }

/*#ifdef __IPHONEOS__ //force rotation on iphone
      int tmp;
      tmp = WINDOW_WIDTH;
      WINDOW_WIDTH = WINDOW_HEIGHT;
      WINDOW_HEIGHT = tmp;
...
#endif*/
      screen = SDL_CreateRGBSurface(0,WINDOW_WIDTH,WINDOW_HEIGHT,VIDEO_BPP,0,0,0,0);
  ...
  if (!fullscreen)
  {
    ...
    #endif
#else
      SDL_CreateWindowAndRenderer(WINDOW_WIDTH, WINDOW_HEIGHT, 0, &sdlWindow, &sdlrenderer);
      if ((sdlWindow == NULL) || (sdlrenderer == NULL)) {
          fprintf(stderr,
                  "\nWarning: couldn't create window and render.\n"
                  "The Simple DirectMedia Layer error that occurred was:\n"
  ...
  if (screen == NULL)
  {
    ...
    if (IOSHasNotch()){
        if (IOSDetectRotation() == 0) {
            x_offset = 0;//40; it's not working disable it for now
        }
    }
...
#else
  font_thread = SDL_CreateThread(load_user_fonts_stub, "fontthread", NULL);
#endif
#endif
  ...
  img_black = SDL_CreateRGBSurface(SDL_SWSURFACE, //MarkTu
  ...
  img_grey = SDL_CreateRGBSurface(SDL_SWSURFACE,  //MarkTu
				  img_btn_off->w, img_btn_off->h,
				  img_btn_off->format->BitsPerPixel,
				  img_btn_off->format->Rmask,
				  img_btn_off->format->Gmask,
...
int main(int argc, char *argv[])
{
...
#if defined(DEBUG) && defined(__MACOSX__)                //EP added block to log messages
  ...
  printf("Seconds in late start-up:  %.3f\n", (double) (time2 - time1) / CLOCK_SPEED);
    ...
    IOSMisc_StartAD(sdlWindow, 0, 0, screen->w, screen_scale);
#endif
