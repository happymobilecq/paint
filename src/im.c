...
#ifndef __BEOS__
#if defined __GLIBC__ && __GLIBC__ == 2 && __GLIBC_MINOR__ >=2 || __GLIBC__ > 2 || __MACOSX__
...
#else
int im_read(IM_DATA* im, char* ks)
{
    int redraw = 0;
    //im->s[0] = L'\0';
    //redraw = SDL_wcslen(ks);
    //printf("total :%zu\n", strlen((const char *)(ks)));
    SDL_utf8strlcpy(im->utf8s, ks, SDL_strlen(ks)+1);
    return redraw;
}
#endif
/* ***************************************************************************
